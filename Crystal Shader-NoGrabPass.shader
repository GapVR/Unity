    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _Repetition)
                UNITY_DEFINE_INSTANCED_PROP( float, _Width)
                UNITY_DEFINE_INSTANCED_PROP( float4, _SurfaceClolr)
                UNITY_DEFINE_INSTANCED_PROP( float, _PasutelColor)
                UNITY_DEFINE_INSTANCED_PROP( float, _ColoeLevel)
                UNITY_DEFINE_INSTANCED_PROP( float, _ColorLoop)
                UNITY_DEFINE_INSTANCED_PROP( fixed, _LightCompletion)
...
////// Specular:
                ...
                float4 _SurfaceClolr_var = UNITY_ACCESS_INSTANCED_PROP( Props, _SurfaceClolr );
                float3 diffuseColor = (_SurfaceClolrtex_var.rgb*_SurfaceClolr_var.rgb); // Need this for specular when using metallic
...
////// Emissive:
                ...
                float _ColorLoop_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ColorLoop );
                float _Repetition_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Repetition );
                float _Width_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Width );
                float node_7509 = step((sin((dot(viewReflectDirection,float3(-0.5,1,0))*_Repetition_var))*0.5+0.5),_Width_var);
                float node_8122 = step((sin((dot(viewReflectDirection,float3(1,0.5,0.5))*_Repetition_var))*0.5+0.5),_Width_var);
                float node_8465 = (step((sin((node_1156*_Repetition_var))*0.5+0.5),_Width_var)*node_7509*node_8122);
                float node_7214 = dot(i.normalDir,float3(0,1,1));
                float node_5582 = (node_8122*step((sin((node_7214*_Repetition_var))*0.5+0.5),_Width_var)*node_7509);
                float node_3443 = saturate(sign((node_8465+node_5582)));
                float _PasutelColor_var = UNITY_ACCESS_INSTANCED_PROP( Props, _PasutelColor );
                float _ColoeLevel_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ColoeLevel );
                float node_4434 = 1.0;
                float3 _LightCompletion_var = lerp( _LightColor0.rgb, lerp(_LightColor0.rgb,float3(node_4434,node_4434,node_4434),step(_LightColor0.r,0.0)), UNITY_ACCESS_INSTANCED_PROP( Props, _LightCompletion ) );
                float3 node_1855 = (lerp(((saturate(3.0*abs(1.0-2.0*frac(((0.5+node_1156)*_ColorLoop_var)+float3(0.0,-1.0/3.0,1.0/3.0)))-1)*node_8465)+(node_5582*saturate(3.0*abs(1.0-2.0*frac((node_7214*_ColorLoop_var)+float3(0.0,-1.0/3.0,1.0/3.0)))-1))),float3(node_3443,node_3443,node_3443),_PasutelColor_var)*_ColoeLevel_var*_LightCompletion_var);
                float3 emissive = saturate(node_1855);
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float3 _LightCompletion_var = lerp( _LightColor0.rgb, lerp(_LightColor0.rgb,float3(node_4434,node_4434,node_4434),step(_LightColor0.r,0.0)), UNITY_ACCESS_INSTANCED_PROP( Props, _LightCompletion ) );
                float3 node_1855 = (lerp(((saturate(3.0*abs(1.0-2.0*frac(((0.5+node_1156)*_ColorLoop_var)+float3(0.0,-1.0/3.0,1.0/3.0)))-1)*node_8465)+(node_5582*saturate(3.0*abs(1.0-2.0*frac((node_7214*_ColorLoop_var)+float3(0.0,-1.0/3.0,1.0/3.0)))-1))),float3(node_3443,node_3443,node_3443),_PasutelColor_var)*_ColoeLevel_var*_LightCompletion_var);
                o.Emission = saturate(node_1855);
                
                float4 _SurfaceClolrtex_var = tex2D(_SurfaceClolrtex,TRANSFORM_TEX(i.uv0, _SurfaceClolrtex));
                float4 _SurfaceClolr_var = UNITY_ACCESS_INSTANCED_PROP( Props, _SurfaceClolr );
                float3 diffColor = (_SurfaceClolrtex_var.rgb*_SurfaceClolr_var.rgb);
                float specularMonochrome;
                float3 specColor;
                float _Metallic_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Metallic );
